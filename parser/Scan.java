...
public class Scan {
    ...
    // returns Token
    public Token scan() {    //this is called when full file is read but still more input
        if( got_eof ) {
        ...
        while(true) {
            if( putback) { //if we are suppose to put a char back
                putback = false;
            }
            else {     // get a new character if the new character is either alpha digit or else switch cases
                c = getchar();
            }
